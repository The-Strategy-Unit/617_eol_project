...
activity %>%
  ...
  theme(legend.position = "none") +
  facet_wrap(vars(group), nrow = 1) +
  labs (x = "Admission Method",
        y = "Critical Care Lenght of Stay")
```
...
local({
  ...
  df %>%
    ...
    geom_segment(data = df_segments %>%
                 ...
                 alpha = 0.4) +
    geom_segment(data = df_segments,
                 aes(xend = proximity_to_death_days, yend = 0),
                 linetype = "dashed",
                 alpha = 0.4) +
    geom_step(lwd = 1) +
    geom_vline(xintercept = 28, linetype = "dotted") +
...
mpi %>%
  filter(group != "(Missing)") %>%
  mutate_at("group", fct_recode, "OTI" = "Other Terminal Illness") %>%
