...
mpi %>%
  filter(location_type != "Unknown", ethnic_group != "Unknown") %>%
  count(ethnic_group, location_type, age_band) %>%
  group_by(ethnic_group, age_band) %>%
  mutate_at("n", ~.x/sum(.x)) %>%
  ggplot(aes(n, ethnic_group)) +
...
```{r forecast activity 5 year bars plot, echo=FALSE, message=FALSE, results='asis'}
...
```{r utilisation curve, message=FALSE,  echo=FALSE, results = "asis"}
utilisation_curve_plot <- function(pod_type, data, loess_span = 0.15) {
  cat("##", ifelse(pod_type == "Bed", "Bed Days", as.character(pod_type)), "\n\n")
...
activity %>%
  ...
  group_nest(pod_type) %>%
  mutate(loess_span = c(0.05, 0.15, 0.15, 0.05, 0.05)) %>% 
  pwalk(utilisation_curve_plot_grouped)
```

```{r utilisation curve location, message=FALSE, echo=FALSE, results = "asis"}
activity %>%
  mutate_at("pod_type",
            ~ifelse(pod_summary_group == "Critical Care Bed Day",
                    "Critical Care Bed Day",
...
```{r utilisation rates, message=FALSE, echo=FALSE, results = "asis"}
list(stp = activity,
     region = activity_region) %>%
  map_dfr(.id = "type", function(.x) {
    .x %>%
  ...
  pwalk(function(pod_type, data) {
    ...
    p <- data %>%
      ...
      geom_point(aes(fill = after_scale(alpha(colour, 0.4))),
                 ...
                 na.rm = TRUE) +
      expand_limits(y = 0) +
      facet_wrap(vars(fct_rev(pod_summary_group))) +
      theme(legend.position = "bottom") +
      labs(colour = "",
           x = "Age of decedent",
           y = "Average use")
...
```{r critical care LoS boxplot age line by cod, message = FALSE, echo = FALSE, results = "asis"}
activity %>%
...
```{r critical care stay theographs, echo = FALSE, message = FALSE, results = "asis"}
activity %>%
  inner_join(select(mpi, su_pat_id, group), by = "su_pat_id") %>%
  filter(pod_summary_group == "Critical Care Bed Day",
         group != "(Missing)") %>%
...
## Who had chemotherapy in their last 4 weeks of life
...
activity %>%
  ...
  geom_col(colour = su_theme_cols()[["orange"]],
           fill = after_scale(alpha(su_theme_cols()[["orange"]], 0.4))) +
  geom_label(aes(y = 0, label = paste0(percent(p, accuracy = 1), " (", comma(nd), ")")),
             vjust = -0.5) +
...
What % of critical care bed days are used by people who died in critical care
versus those who did not die in critical care?

```{r cc mekko plots, message=FALSE, warning=FALSE}
aligned <- local({
...
What % of critical care bed days are used by elective admissions versus
...
tlos %>%
  filter(proximity_to_death_days <= 42) %>%
  mutate(n = (proximity_to_death_days+1) * stp) %>%
...
## cumulative bed days for terminal spells

```{r tlos pcnt bed days plot cumulative}
tlos %>%
  pivot_longer(c(stp, region)) %>%
  group_by(name) %>%
  arrange(-proximity_to_death_days) %>%
  mutate(n = (proximity_to_death_days+1) * value,
         p = cumsum(n) / sum(n),
         dropline = ifelse(proximity_to_death_days == 7, p, NA)) %>%
  filter(proximity_to_death_days <= 140) %>%
  ggplot(aes(proximity_to_death_days, p)) +
  geom_segment(aes(xend = proximity_to_death_days, y = dropline, yend = 0),
               na.rm = TRUE,
               linetype = "dashed") +
  geom_step(aes(colour = name)) +
...
## cumulative bed days as a percentage of all emergency bed days

```{r tlos pcnt bed days plot cumulative - all activity}
tlos %>%
  pivot_longer(c(stp, region)) %>%
  inner_join(by = "name",
             list(stp = activity, region = activity_region) %>% 
               map(filter, pod_summary_group %in% c(#"Elective Admission Bed Day", 
                                                    "Emergency Admission Bed Day")) %>% 
               map_dbl(nrow) %>% 
               enframe(value = "total_beddays")) %>%
  group_by(name) %>%
  arrange(-proximity_to_death_days) %>%
  mutate(n = (proximity_to_death_days+1) * value,
         p = cumsum(n) / total_beddays, #sum(n),
         dropline = ifelse(proximity_to_death_days == 7, p, NA)) %>%
  filter(proximity_to_death_days <= 140) %>%
  ggplot(aes(proximity_to_death_days, p)) +
  geom_segment(aes(xend = proximity_to_death_days, y = dropline, yend = 0),
