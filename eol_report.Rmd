...
bind_rows(
  ...
  group_by(sex, type) %>%
  summarise(density = list(density(age, from = 18, to = 110, cut = 1))) %>%
  ...
  geom_density(aes(age, region),
               ...
               linetype = "dashed") +
  coord_flip() +
  scale_colour_manual(values = c("male" = su_theme_cols()[["dark_blue"]],
  ...
  theme(axis.text.x = element_blank(),
        ...
        legend.justification = c(1, 0)) +
  labs(x = "age", y = "", colour = "") +
  annotate("label", 107.5, -1.0, label = "Males", hjust = 0,
           colour = su_theme_cols()[["dark_blue"]],
           fill = after_scale(alpha(su_theme_cols()[["dark_blue"]], 0.4))) +
  annotate("label", 107.5,  1.0, label = "Females", hjust = 1,
           colour = su_theme_cols()[["dark_red"]],
           fill = after_scale(alpha(su_theme_cols()[["dark_red"]], 0.4)))
```
...
activity %>%
  ...
  pmap(function(group, data) {
    data %>%
      ...
      scale_fill_manual(values = c(
        "Bed" = su_theme_cols()[["orange"]],
        "Planned Contact" = su_theme_cols()[["blue"]],
        "Planned Admission" = su_theme_cols()[["red"]],
        "Urgent Service Event" = su_theme_cols()[["charcoal"]]
      )) +
  ...
  theme(legend.direction = "vertical") -> pathways_plots
...
#        width = 29.7,
#        dpi = "retina",
#        type = "cairo"); clipr::write_clip(tf)
...
tibble(proximity_to_death_days = activity %>%
  ...
  scale_x_proximity_to_death_days() +
  labs(x = "Proximity to death (Months)",
       y = "Percentage of decedents who had chemo (by earliest chemo date)")
```
