...
mpi %>%
  transmute(su_pat_id,
            age,
            year = list(c("Year of death", "Year prior to death"))) %>%
  filter(age <= 90) %>%
  unnest(year) %>%
  left_join(activity %>% 
              filter(chemotherapy_indicator == 1) %>%
              mutate(year = ifelse(proximity_to_death_days <= 365,
                                   "Year of death",
                                   "Year prior to death")) %>%
              distinct(su_pat_id, year) %>%
              mutate(value = TRUE),
            by = c("su_pat_id", "year")) %>%
  modify_at("value", replace_na, FALSE) %>%
  group_by(year, age) %>%
  summarise_at("value", mean) %>%
  ggplot(aes(age, value, colour = year)) +
  geom_smooth(aes(fill = after_scale(alpha(colour, 0.4))),
              method = "loess",
              span = 0.4,
              formula = y ~ x) +
  scale_y_continuous(labels = percent) +
