...
activity %>%
  ...
  labs(x = "Died in Critical Care?",
       y = "Critical Care Length of Stay")
```
...
```{r chemo in last month of life binomial ci plot}
...
activity %>%
  ...
  #left_join(cwt_groups, by = c("primary_cod" = "icd10")) %>%
  #mutate_at("cancer_site", replace_na, "Non-Cancer") %>%
  inner_join(cwt_groups, by = c("primary_cod" = "icd10")) %>%
  mutate(last_month = ifelse(proximity_to_death_days <= 28, su_pat_id, NA)) %>%
  mutate_at("cancer_site",
            fct_relevel,
            unique(cwt_groups$cancer_site)) %>% #,
  #"Non-Cancer") %>%
  mutate_at("cancer_site",
            fct_recode,
            "Haematological" = "Haematological (Excluding Acute Leukaemia)",
            "Urological (Excluding Prostate)" =
              "Urological (Excluding Testicular and Prostate)",
            "Urological (Excluding Prostate)" =
              "Testicular",
            "Head & Neck" = "Thyroid",
            "Metastatic disease of unknown primary" = "Other") %>%
  ...
  ggplot(aes(fct_rev(cancer_site), est)) +
  geom_errorbar(aes(ymin = lwr.ci, ymax = upr.ci),
                colour = su_theme_cols("orange")) +
  geom_point(colour = su_theme_cols("orange"),
             fill = after_scale(alpha(su_theme_cols("orange"), 0.4))) +
