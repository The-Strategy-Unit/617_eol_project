...
forecast_data %>%
  select(-est_deaths) %>%
  mutate_at("activity", comma) %>%
  pivot_wider(names_from = "year",
              values_from = "activity") %>%
  arrange(pod_type, age_band) %>%
  group_by(pod_type) %>%
  mutate(rn = row_number()) %>%
  ungroup() %>%
  mutate_at("pod_type", ~if_else(rn == 1, as.character(.x), "")) %>%
  select(pod_type, age_band, `2020`:`2040`)
```
...
activity %>%
  ...
  group_nest(pod_type) %>%
  mutate(loess_span = 0.05) %>% #c(0.05, 0.15, 0.15, 0.05, 0.05)) %>% 
...
tlos <- activity %>%
  ...
  count(proximity_to_death_days) %>%
  filter(proximity_to_death_days <= 70)
...
tlos %>%
  ...
  labs(x = "Proximity to Death (in days) = LoS",
...
tlos %>%
  ...
  scale_y_continuous(expand = expansion(c(0, 0.05)),
                     ...
                     position = "right") +
  facet_wrap(vars(pod_summary_group), ncol = 1, scales = "free") +
  labs(x = "Proximity to Death (in days)",
       y = "Bed days as % of all bed days for terminal spell")
```
