...
# text above
basic_stats.fmt <- do.call(rbind, transpose(basic_stats))

basic_stats.fmt <- c(
  comma(basic_stats.fmt["deaths",] %>% unlist()),
  comma(basic_stats.fmt["population",] %>% unlist()),
  number(basic_stats.fmt["est",] %>% unlist(), accuracy = 0.1),
  number(basic_stats.fmt["lwr.ci",] %>% unlist(), accuracy = 0.1),
  number(basic_stats.fmt["upr.ci",] %>% unlist(), accuracy = 0.1),
  number(basic_stats.fmt["mean_age",] %>% unlist(), accuracy = 0.1),
  percent(basic_stats.fmt["0-17",] %>% unlist(), accuracy = 0.1),
  percent(basic_stats.fmt["18-64",] %>% unlist(), accuracy = 0.1),
  percent(basic_stats.fmt["65-74",] %>% unlist(), accuracy = 0.1),
  percent(basic_stats.fmt["75-84",] %>% unlist(), accuracy = 0.1),
  percent(basic_stats.fmt["85+",] %>% unlist(), accuracy = 0.1)
) %>%
...
p + geom_text(aes(label = percent(stp, accuracy = 1)),
              size = 3.5,
              vjust = -.35)
```
...
p <- bind_rows(
  ...
  mutate_at("pod_type",
            fct_relevel,
            c(levels(activity$pod_type), "Critical Care Bed Day")) %>%
  mutate_at("pod_summary_group", ~ifelse(pod_type == "Critical Care Bed Day",
...
pcnt_bar_comparison_plot <- function(mpi, category, value, reorder = TRUE) {
  data <- mpi %>%
    ...
    group_by({{category}}) %>%
    #filter(group != "(Missing)") %>%
    #modify_at("died_in_critical_care", replace_na, 0) %>%
...
ipbd <- activity %>%
  ...
  complete(pod_summary_group, proximity_to_death_days, fill = list(n = 0)) %>%
  ungroup() %>%
  inner_join(activity %>%
...
activity %>%
  ...
  count(pod_type, pod_summary_group, activity_year, age,
        name = "activity") %>%
  arrange(pod_type, pod_summary_group, activity_year, age) %>%
...
activity_ambulance %>%
  count(proximity_to_death_days, arrival_mode) %>%
...
activity_ambulance %>%
  group_by(arrival_mode, proximity_to_death_days, group) %>%
  summarise_at("n", sum) %>%
  ungroup() %>%
