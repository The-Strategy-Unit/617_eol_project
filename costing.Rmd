...
activity_costs_region <- file.path("data", "sensitive", "activity_costs.fst") %>%
  read_fst() %>%
  as_tibble() %>%
...
activity_costs <- activity_costs_region %>%
  semi_join(mpi, by = "su_pat_id")

money_format <- comma_format(prefix = "£", scale = 1e-6, suffix = "m", accuracy =  1)
```
...
local({
  ...
  df %>%
    pwalk(function(pod_type, data) {
      ...
      pwalk(data, function(pod_summary_group, data) {
        cat ("###", as.character(pod_summary_group), "\n\n")
        
        p <- ggplot(data, aes(cost, simple_ss_flag, colour = simple_ss_flag)) +
          geom_density_ridges(aes(fill = after_scale(alpha(colour, 0.4)))) +
          scale_x_continuous(labels = comma_format(prefix = "£"),
                       trans = "log10",
                       expand = expansion(c(0.1, 0.05))) +
...
bind_rows(
  activity_costs %>%
    filter(pod_type != "Bed", cost > 0) %>%
    group_by(proximity_to_death_days) %>%
    summarise(across(cost, mean), pod_type = "All Activity", .groups = "drop"),
  activity_costs %>%
    filter(pod_type != "Bed", cost > 0) %>%
    group_by(pod_type, proximity_to_death_days) %>%
    summarise(across(cost, mean), .groups = "drop")
) %>%
  nest_by(pod_type) %>%
  pwalk(function(pod_type, data) {
    cat("##", as.character(pod_type), "\n\n")
    ...
    p <- data %>%
      ggplot(aes(proximity_to_death_days, cost)) +
      geom_point(colour = su_theme_cols("orange"),
                 fill = after_scale(alpha(su_theme_cols("orange"), .4)),
                 shape = "circle filled") +
      ...
      scale_y_continuous(position = "right",
                         labels = comma_format(accuracy = 1, prefix = "£")) +
      labs(x = "Proximity to Death (Months)",
           y = "Average Activity Cost")
...
```{r, results = "asis"}
spend_plot("All Activity",
           activity_costs_ex_bed %>%
             filter(ss_flag == "Spec. Services",
                    proximity_to_death_days > 0))

activity_costs_ex_bed %>%
  filter(ss_flag == "Spec. Services", proximity_to_death_days > 0) %>%
  group_nest(pod_type) %>%
  pwalk(spend_plot)
```
...
activity_costs %>%
  filter(ss_flag %in% c("CCG", "Spec. Services")) %>%
  semi_join(ccg_pod_types, by = "pod_type") %>%
  group_by(ss_flag, pod_type) %>%
  
  filter(cost > quantile(cost, 0.25), cost < quantile(cost, 0.75)) %>%
...
bind_rows(
  ...
  pwalk(function(pod_type, data) {
    ...
    lbls <- data %>%
      ...
      summarise_at("cost", sum) %>%
      arrange(year) %>%
      mutate(act_sum_lbl = comma(cost, prefix = "£", accuracy = 1),
