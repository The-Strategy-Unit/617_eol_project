...
activity_costs %>%
  ...
  ggplot(aes(cost, pod_type, colour = fct_rev(simple_ss_flag))) +
  geom_col(aes(fill = after_scale(alpha(colour, 0.4))),
           position = "dodge") +
  scale_x_continuous(labels = comma_format(prefix = "£")) +
  scale_y_discrete(expand = expansion()) +
...
```{r}
activity_costs %>%
  filter(!pod_type %in% c("Bed", "Critical Care")) %>%
  group_by(pod_type) %>%
  ...
  #mutate(label = ifelse(name == "ss", percent(value), NA)) %>%
...
activity_costs %>%
  ...
  ggplot(aes(proximity_to_death_days, cost)) +
  geom_step() +
  geom_abline(slope = 1/(365*2), intercept = 1, colour = "lightgrey") +
  geom_polygon(data = tibble(x = c(Inf, 729, 0, 0,   Inf, Inf),
                             y = c(0,   0,   1, Inf, Inf, 0)),
               aes(x, y),
               colour = NA,
               fill = "white") +
