...
```{r}
list(stp = activity_costs, region = activity_costs_region) %>%
...
bind_rows(
  ...
  pmap(function(pod_type, data) {
    lbls <- data %>%
      ...
      summarise_at("cost", sum) %>%
      arrange(year) %>%
      mutate(act_sum_lbl = money_format(cost),
             pcnt_change_lbl = percent(ifelse(year == 2020,
                                              NA,
                                              cost / first(cost) - 1),
                                       accuracy = .1))
...
activity_costs %>%
  ...
  geom_text(aes(label = comma(cost, prefix = "£")),
                vjust = -0.35) +
  scale_y_continuous(labels = money_format,
                     expand = expansion(c(0, 0.05))) +
  labs(x = "", y = "")
...
mpi_region %>%
  ...
  unnest_wider(ci) %>%
  inner_join(as_tibble(stp_geo) %>% select(stp18cd, stp18nm), by = c("stp" = "stp18cd")) %>%
