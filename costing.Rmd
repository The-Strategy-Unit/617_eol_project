...
average_costs_per_person <- activity_costs %>%
  ...
  summarise(cost = sum(cost),
            #decedents = n_distinct(su_pat_id),
            .groups = "drop") %>%
  inner_join(count(mpi, age, name = "decedents"), by = "age") %>%
  complete(nesting(pod_type),
           activity_year = 0:1,
           age = seq(18, 90),
           sex,
           fill = list(cost = 0, decedents = 0))
...
forecast_deaths %>%
  ...
  mutate_at("age_group",
            ...
            right = FALSE) %>%
  bind_rows(mutate_at(., "pod_type", ~"All Activity"), .) %>%
  group_by(year, age_group, pod_type) %>%
  summarise(across(c("est_deaths", "cost", "decedents"), sum), .groups = "drop") %>%
  mutate(cost = ifelse(decedents, cost / decedents * est_deaths, 0)) %>%
  select(-est_deaths, -decedents) %>%
  group_nest(pod_type) %>%
  pmap(function(pod_type, data) {
    lbls <- data %>%
      group_by(year) %>%
